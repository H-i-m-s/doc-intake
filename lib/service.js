...
export function parsePythonOutput(stdout, settings) {
  try {
    ...
    throw new DocIntakeError(
      `Python 输出解析失败: ${parseError.message}`,
      {
        code: "PYTHON_OUTPUT_PARSE_FAILED",
        cause: parseError,
        details: {
          stdout: sanitizeProcessOutput(stdout, settings).slice(0, 200),
        },
      },
    );
  }
...
export async function extractDocument(input, ctx) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", code => {
      ...
      if (code === 0) {
        try {
          const result = parsePythonOutput(stdout, mergedSettings);
