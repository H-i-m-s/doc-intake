...
本模块取代原先的 mtef.py（zhexiao/mtef-go 的 Python 移植，已删除）。取代它的原因
不是风格，而是它有多处与规范不符的读取长度：读错长度之后 reader 会停在不该停的位置，
而它仍然继续往下解析，于是得到一段看起来合理的结果。另外它把 v5 当"平铺流 +
makeAST 事后重建嵌套"处理，与规范相反。
...
3. 合成字节：小值/大值/负值 nudge、SIZE 三种情形、两字节变体号、FUTURE 跳过长度
   ——这几条范例与语料都没覆盖到。
自测：python test/selftest_v5_parse.py
...
_SPEC_PARTS = [
    ...
    # 抄错任何一个字节都会在 spec_example() 里被断言拦住（那个函数现在在 test/selftest_v5_parse.py 里）。
